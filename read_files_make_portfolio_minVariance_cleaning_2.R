...
print("READY .. reading input file")
...
for (Exp in 10:11)
{
    ...
    Tcovariance <- 100*Exp;                                         # no days of history used to calculate Covariance matrix
...
#    for (i in 0:(noReweighting-1))
...

    {
        ...
        CorrMatrix <- cor(returns[(i*noDaysBetweenReweigting+1):(i*noDaysBetweenReweigting + Tcovariance),])
        In[1:NoStocks] <- 1;
        
        #
        #   remove noise with Random Matrix theory
        #


        EigCorr <- eigen(CorrMatrix, symmetric=T, only.values = F);

        minX <- 0.0
        maxX <- ceiling(max(EigCorr$values))+1.01


        ee <- EigCorr$values;
        EigenVals <- ee;
        EigenVec <- EigCorr$vectors;
        ...
        indicator <- 1:NoStocks
        indicatorInformation <- indicator[EigenVals > LambdaMax]
        indicatorNoInformation <- indicator[EigenVals < LambdaMax]

        percentageExplained <- sum(EigenVals[EigenVals > LambdaMax])/sum(EigenVals)



        #
        #   Determine min variance portfolio
        #
    ...
    # stockReturns  = port[(Tcovariance+noDaysBetweenReweigting*(1:noReweighting)),]/port[(Tcovariance+noDaysBetweenReweigting*(0:(noReweighting-1))+1),]
     ...
     reinvest[1:(Tcovariance+noDaysBetweenReweigting+1)] <- 1
    ...
    #
     ...
     #   reinvest[(Tcovariance+(i*noDaysBetweenReweigting)+1):((i+1)*noDaysBetweenReweigting)] <- reinvest[(i*noDaysBetweenReweigting)] * stockReturns2[i-firstTimeReweighting]
     #}
     
     for (i in 1:noReweighting)
     {
        print(c(i,(Tcovariance+(i*noDaysBetweenReweigting)+1),(Tcovariance+(i+1)*noDaysBetweenReweigting)))
        reinvest[(Tcovariance+(i*noDaysBetweenReweigting)+1):(Tcovariance+(i+1)*noDaysBetweenReweigting)] <- reinvest[Tcovariance+(i*noDaysBetweenReweigting)] * stockReturns2[i]
    ...
    #close(outputFile); 
     ...
     varPort500 = var(returns1NPort[500:Tsample])*252
     varPort1000 = var(returns1NPort[1000:Tsample])*252
 ...
 #    plot(as.Date(aa[Tstart:Tsample,1],"%d-%b-%Y"),reinvest[Tstart:Tsample],type="l",ylab="Portfolio Value",xlab="Year",ylim=c(yMin,yMax),col='green')
     ...
     cat(c("Tcov",Tcovariance, "time reweighting",noDaysBetweenReweigting,"min variance",varPort,"min variance",varPort,varPort500,varPort1000, "\n") , file = ExpFile, sep = ",",append = TRUE)
