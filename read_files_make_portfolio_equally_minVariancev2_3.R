...
underlyingNoStocks <- matrix(0,NoObs,NoFunds);
portVal <- numeric(NoObs)
...
for (i in 0:(noReweighting-1))
{
    ...
    for (j in 1:NoFunds)    # keep no. stocks constant in between reweighting
    {
...
for (i in ((noReweighting-1)*noDaysBetweenReweigting+1):NoObs)
{
    underlyingNoStocks[i,] <- underlyingNoStocks[(i-1),]        
}

portReturns <- numeric(NoObs)
